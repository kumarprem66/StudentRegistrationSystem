...
public class CourseImpl implements CourseInterface{
	...
	public String addCourse(Course course, Map<Integer, Course> courses) {
		...
		if(courses.containsValue(course)) {
			return "Course is already available";
		}else {
...
//			List<Course> courseList = (List<Course>)courses.values();
	...
	@Override
	public void searchCourseDuration(Map<Integer, Course> courses, int mon) throws EmptyException {
		// TODO Auto-generated method stub
		
		int temp = 0;
		for(Map.Entry<Integer, Course> kv : courses.entrySet()) {
			String dur = kv.getValue().getDuration_of_course();
			int month = Integer.valueOf(dur.split("-")[0]);
			if(month<=mon) {
				temp = 1;
				System.out.println(kv.getValue());
			}
		...
		if(temp==0) {
			throw new EmptyException("No course available with this duration");
